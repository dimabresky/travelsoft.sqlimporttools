...
try {
    if ($request->isPost() && check_bitrix_sessid()) {
        ...
        if ($files->upload($request->getFile("SQL"))) {
            ...
            $_SESSION["travelsoft_sqlimporttools_SUCCESS_UPLOAD_FILE"] = true;
            reload();
        }
    }
...
if (isset($_SESSION["travelsoft_sqlimporttools_SUCCESS_UPLOAD_FILE"]) && $_SESSION["travelsoft_sqlimporttools_SUCCESS_UPLOAD_FILE"]) {
    CAdminMessage::ShowNote(Loc::getMessage("travelsoft_sqlimporttools_SUCCESS_UPLOAD_FILE"));
    unset($_SESSION["travelsoft_sqlimporttools_SUCCESS_UPLOAD_FILE"]);
}
...
$o_tab = new CAdminTabControl("travelsoft_sqlimporttools_tab_control", array(
    array(
        "DIV" => "travelsoft_sqlimporttools",
        "TAB" => Loc::getMessage("travelsoft_sqlimporttools_TAB_TITLE"),
        "ICON" => "",
        "TITLE" => Loc::getMessage("travelsoft_sqlimporttools_TAB_SUBTITLE")
    )
        ));
?>
<form action="<? $APPLICATION->GetCurPage() ?>" method="post" enctype="multipart/form-data">
    ...
    ?>
    <tr>
        <td width="40%"><label><?= Loc::getMessage("travelsoft_sqlimporttools_SQL_FILE_FIELD_TITLE") ?>:</label></td>
        <td width="60%">
    ...
    <? $o_tab->Buttons();
    ?>
    <input type="submit" name="next" value="<?= Loc::getMessage("travelsoft_sqlimporttools_SAVE_BTN_TITLE") ?>" title="<?= Loc::getMessage("dki_VKEXPORTER_NEXT_BTN_TITLE") ?>" class="adm-btn-save">
    <?
    $o_tab->End();
    ?>
